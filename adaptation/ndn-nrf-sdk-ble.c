...
#include "ndn-nrf-sdk-ble.h"

// forward declaring some functions
int
advertising_init(const uint8_t *payload, uint32_t payload_len);
 ...
 */
static bool
config_is_valid_custom(ble_adv_modes_config_t const * const p_config)
{
...
uint32_t
ble_advertising_init_custom(ble_advertising_t            * const p_advertising,
 ...
 */
int
nrf_sdk_ble_adv_start(const uint8_t *payload, uint32_t payload_len)
{
 ...
 */
int
nrf_sdk_ble_scan_start(void (*scan_callback)(const uint8_t *scan_data, uint8_t scan_data_len))
{
