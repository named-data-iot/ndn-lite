...
ndn_key_storage_init(void)
{
  ...
  for (uint8_t i = 0; i < NDN_SEC_SIGNING_KEYS_SIZE; i++) {
    storage.ecc_pub_keys[i].key_size = NDN_SEC_INVALID_KEY_SIZE;
    storage.ecc_prv_keys[i].key_size = NDN_SEC_INVALID_KEY_SIZE;
    storage.hmac_keys[i].key_size = NDN_SEC_INVALID_KEY_SIZE;

    if (i <= NDN_SEC_ENCRYPTION_KEYS_SIZE)
      storage.aes_keys[i].key_size = NDN_SEC_INVALID_KEY_SIZE;
  }
  return &storage;
}
...
{
  for (uint8_t i = 0; i < NDN_SEC_SIGNING_KEYS_SIZE; i++) {
    if (storage.hmac_keys[i].key_size == NDN_SEC_INVALID_KEY_SIZE) {
...
{
  for (uint8_t i = 0; i < NDN_SEC_SIGNING_KEYS_SIZE; i++) {
    if (storage.ecc_pub_keys[i].key_size == NDN_SEC_INVALID_KEY_SIZE) {
...
ndn_key_storage_get_empty_aes_key(ndn_aes_key_t** aes)
{
  for (uint8_t i = 0; i < NDN_SEC_ENCRYPTION_KEYS_SIZE; i++) {
    if (storage.aes_keys[i].key_size == NDN_SEC_INVALID_KEY_SIZE) {
