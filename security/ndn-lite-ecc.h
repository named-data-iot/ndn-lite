...
/**
 * Encode a signature in the format generated by the micro-ecc library according
 * to the ASN.1 standard.
 * @param sig_val. Input. Pointer to signature to be encoded.
 * @param sig_buf_size. Input. Size of the un-encoded signature.
 * @param sig_buf_max_size. Input. Size of the buffer the un-encoded signature is stored in.
 *                            If the buffer the signature is stored in is not also large
 *                            enough to hold the encoded signature, the function will fail.
 * @param encoded_sig_size. Output. Size of the encoded signature, if encoding is successful.
 * @param ecdsa_type. Input. Type of ECDSA signature. Currently only secp256r1 is supported.
 * @return NDN_SUCCESS if there is no error.
 */
int
