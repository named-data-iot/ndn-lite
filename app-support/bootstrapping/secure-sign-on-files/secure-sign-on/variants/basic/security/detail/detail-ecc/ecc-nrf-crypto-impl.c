...
int sign_on_basic_nrf_crypto_gen_ec_keypair(uint8_t *pub_key_buf, uint32_t pub_key_buf_len, uint32_t *pub_key_output_len,
    uint8_t *pri_key_buf, uint32_t pri_key_buf_len, uint32_t *pri_key_output_len,
    uECC_Curve curve) {
  ...
  *pub_key_output_len = (uint32_t) raw_pub_key_size;
  memcpy(pri_key_buf, raw_priv_key, raw_priv_key_size);
  *pri_key_output_len = (uint32_t) raw_priv_key_size;
...
int sign_on_basic_nrf_crypto_gen_sha256_ecdsa_sig(const uint8_t *pri_key_raw, uECC_Curve curve,
  ...
  const uint32_t m_digest_len = NRF_CRYPTO_HASH_SIZE_SHA256;
  ...
  uint32_t pri_key_raw_len = uECC_curve_private_key_size(curve);
...
int sign_on_basic_nrf_crypto_gen_ecdh_shared_secret(
    const uint8_t *pub_key_raw, uint32_t pub_key_raw_len,
