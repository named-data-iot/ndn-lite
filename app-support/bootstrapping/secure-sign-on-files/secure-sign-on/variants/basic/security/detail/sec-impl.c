...
int sign_on_basic_gen_sha256_ecdsa_sig(const uint8_t *pri_key_raw, uECC_Curve curve,
  ...
  if (ndn_ecdsa_sign(payload, payload_len, output_buf, output_buf_len, pri_key_raw, 
                 pri_key_raw_len, ndn_ecc_curve, output_len) != NDN_SUCCESS) {
    return SIGN_ON_BASIC_SEC_OP_FAILURE;
  }
  uint32_t encoded_sig_size;
  if (ndn_ecdsa_asn_encode_raw_sig(output_buf, *output_len, output_buf_len, 
                                   &encoded_sig_size, ndn_ecc_curve) != NDN_SUCCESS) {
    
    return SIGN_ON_BASIC_SEC_OP_FAILURE;
  }
  *output_len = encoded_sig_size;
  APP_LOG_HEX("In sign_on_basic_gen_sha256_ecdsa_sig, value of encoded signature "
              "generated by ndn_asn_encode_raw_ecdsa_sig:", output_buf, *output_len);
  return SIGN_ON_BASIC_SEC_OP_SUCCESS;
}
