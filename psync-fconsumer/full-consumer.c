                         ...
                         void (*onUpdate)(void*))
{
    ...
    error_reporting(ndn_name_from_string(&consumer->m_syncPrefix, syncPrefix, sizeof(syncPrefix)));
    // ndn_name_print(&consumer->m_syncPrefix);
    
    // construct empty IBLT
    const char* ibf = "/x%9Cc%60%18%05%A3%60%F8%00%00%02%D0%00%01";
    error_reporting(ndn_name_from_string(&consumer->m_iblt, ibf, sizeof(ibf)));
